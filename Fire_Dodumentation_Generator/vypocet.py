...
def vypocet_rizika(cesta, soubor, root):
    ...
    POP = wb["POP"]  # Opens the sheet called POP
    konst = wb["input konstrukce"]
    ###########################################################################
    ...
    PU = wb["PU"]  # Opens the {sheet} called POP
    ...
    df = xl.parse("k pod 500", index_col=0)
    df1 = xl.parse("k nad 500 pod 36", index_col=0)
    df2 = xl.parse("k nad 500 nad 36", index_col=0)
    ...
    for i in range(0, len(S_m)):
        for m in range(0, 2):
            if m == 0:
                if S_m[i] <= 500:
                    val_k[i, m] = df.loc[y_idx_k[i, m], x_idx_k[i, m]]
                if S_m[i] > 500:
                    if PU_hs[i] < 3.6:
                        val_k[i, m] = df1.loc[y_idx_k[i, m], x_idx_k[i, m]]
                    if PU_hs[i] > 3.6:
                        val_k[i, m] = df2.loc[y_idx_k[i, m], x_idx_k[i, m]]
            if m == 1:
                if S_m[i] <= 500:
                    val_k[i, m] = df.loc[y_idx_k[i, m-1], x_idx_k[i, m]]
                if S_m[i] > 500:
                    if PU_hs[i] < 3.6:
                        val_k[i, m] = df1.loc[y_idx_k[i, m-1], x_idx_k[i, m]]
                    if PU_hs[i] > 3.6:
                        val_k[i, m] = df2.loc[y_idx_k[i, m-1], x_idx_k[i, m]]
    for i in range(0, len(S_m)):
        for n in range(0, 2):
            if n == 0:
                if S_m[i] <= 500:
                    val_k[i+len(S_m), n] = df.loc[y_idx_k[i, n+1], x_idx_k[i, n]]
                if S_m[i] > 500:
                    if PU_hs[i] < 3.6:
                        val_k[i+len(S_m), n] = df1.loc[y_idx_k[i, n+1],
                                                      x_idx_k[i, n]]
                    if PU_hs[i] > 3.6:
                        val_k[i+len(S_m), n] = df2.loc[y_idx_k[i, n+1],
                                                      x_idx_k[i, n]]
            if n == 1:
                if S_m[i] <= 500:
                    val_k[i+len(S_m), n] = df.loc[y_idx_k[i, n], x_idx_k[i, n]]
                if S_m[i] > 500:
                    if PU_hs[i] < 3.6:
                        val_k[i+len(S_m), n] = df1.loc[y_idx_k[i, n], x_idx_k[i, n]]
                    if PU_hs[i] > 3.6:
                        val_k[i+len(S_m), n] = df2.loc[y_idx_k[i, n], x_idx_k[i, n]]
    # print(val_k)
    # len(S_m)
    ...
    ###########################################################################
    'Modifikace normou spočtených provozů'
    ###########################################################################
    Info = wb["Info"]  # Opens the sheet called Info
    ...
    for i in range(0, len(pol1)):
        if pol1[i] in list_avaiable or\
           pol1[i] == 'OB2 byt' or\
           pol1[i] == 'OB3' or pol1[i] == 'OB4 ubyt.' or pol1[i] == 'OB4 sklad' or\
           pol1[i] == 'CHÚC-A' or pol1[i] == 'CHÚC-B' or\
           pol1[i] == 'CHÚC-C' or pol1[i] in list_avaiable2:
            check.append(i)

    if len(check) > 0:
        for item in check:
            if pol1[item] == 'OB2 byt':
                if (float(p_only[item]) - float(p_n[item])) >= 0:
                    p[item] = '%.2f' % 40
                if (float(p_only[item]) - float(p_n[item])) > 5:
                    p[item] = '%.2f' % 45.00
                if (float(p_only[item]) - float(p_n[item])) >= 15:
                    p[item] = '%.2f' % 50.00
                a[item] = str('%.2f' % 1.00)
                # pozn = ' $^{1)}$'
                # data_used[item][0] = NoEscape(data_used[item][0] + pozn)
            if pol1[item] in list_avaiable:
                if 0 <= float(p_s[item]) <= 5:
                    p[item] = '%.2f' % float(p_n[item])
                if float(p_s[item]) > 5:
                    p_n[item] = ((float(p_s[item]) - 5) * 1.15)\
                                            + float(p_n[item])
                    p[item] = '%.2f' % p_n[item]
                a[item] = str('%.2f' % 1.00)
            if pol1[item] in list_avaiable2:
                p[item] = '%.2f' % float(p_n[item])
                a[item] = a_n[item]
            if pol1[item] == 'OB3' or pol1[i] == 'OB4 ubyt.':
                p[item] = '%.2f' % float(p_n[item])
                a[item] = str('%.2f' % 1.00)
            if pol1[item] == 'OB4 sklad':
                p[item] = '%.2f' % float(p_n[item])
                a[item] = str('%.2f' % 1.00)
    ...
    [delka_PU, sirka_PU] = rozmer(np.round(a, decimals=2), type_sys, podlazi, h_p)
    ##########################################################################
    ###########################################################################
    ...
    indices = []
    for item in Jmeno:
        indices.append(jmeno.index(item))

    '''indices = [i for i, x in enumerate(log_p_n) if x == 1]'''
    pol_res = []
    for item in indices:
        pol_res.append(pol[item])

    for i in range(0, len(p)):
        list_res[i].append(Jmeno[i])
        list_res[i].append(float(delka_PU[i]))
        list_res[i].append(float(sirka_PU[i]))
        list_res[i].append(pol_res[i])
